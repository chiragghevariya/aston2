    ...
    @if($submits->count() > 0)
        <table class="table  aston-datatable text-center">
            ...
            @foreach($submits as $submit)
                <tr>
                    <td class="text-center"> @if($submit->status=="4") <i class="fa fa-close"></i>
                        @elseif($submit->status=="3") <i class="fa fa-check" aria-hidden="true"></i>
                        @else
                            <i class="fa fa-exclamation" aria-hidden="true"></i>
                        @endif </td>

                        <td>{{$submit->user()->student()->enrollno}}</td>
